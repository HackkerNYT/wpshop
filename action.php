...
if(isset($_POST["category"])){
    ...
    //Hiển thị chữ "Categories"
    echo "    
            <div class='aside'>
            <h3 class='aside-title'>Categories</h3>
            <div class='btn-group-vertical'>
	";
    //Hiển thị dữ liệu category và số lượng product thuộc category
    if(mysqli_num_rows($run_query) > 0){
        $i=1;
        while($row = mysqli_fetch_array($run_query)){
            $cid = $row["cat_id"];
            $cat_name = $row["cat_title"];
            //Đếm số lượng product thuộc category
...
if(isset($_POST["brand"])){
    ...
    if(mysqli_num_rows($run_query) > 0){
        ...
        while($row = mysqli_fetch_array($run_query)){
            ...
            //Đếm số lượng product thuộc brand
...
//Phân trang (mỗi trang 9 product)
if(isset($_POST["page"])){
...
//Hiển thị product theo category và theo trang (mỗi trang 9 product)
...
//Lấy product theo category hoặc brand hoặc ô tìm kiếm
...
//Thêm product vào cart (giỏ hàng)
if(isset($_POST["addToCart"])){
    $p_id = $_POST["proId"];
    if(isset($_SESSION["uid"])){//Nếu đã login thì tìm theo user id
        $user_id = $_SESSION["uid"];
        $sql = "SELECT * FROM cart WHERE p_id = '$p_id' AND user_id = '$user_id'";
        $run_query = mysqli_query($con,$sql);
        $count = mysqli_num_rows($run_query);
        if($count > 0){//Product đã được chọn, không chọn lại nữa
            echo "
            ...
            ";
        } else {//Chưa trùng, insert vào cart
     ...
     }else{//Nếu chưa login thì tìm theo ip address
...
//Đếm số product được người dùng chọn trong cart 
if (isset($_POST["count_item"])) {
    //Nếu người dùng login thì dùng user id
    if (isset($_SESSION["uid"])) {
        $sql = "SELECT COUNT(*) AS count_item FROM cart WHERE user_id = $_SESSION[uid]";
    }else{
    //Nếu chưa login thì dùng ip address
...
//Hiển thị cart trên Dropdown menu
